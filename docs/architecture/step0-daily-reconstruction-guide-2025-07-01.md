...
**비즈니스 시나리오**
- **AWS 비용 최적화**를 위한 일일 리소스 해제 후 재구축
- **Step 6 Infrastructure as Code** 기반으로 **30분 내 전체 스택 복원** 목표

**기술적 요구사항**
- ✅ **Terraform 완료 상태**에서 시작 (EC2, VPC, Security Groups 준비됨)
- ✅ **깨끗한 Kubernetes 클러스터** (kubeadm, Calico CNI 구성 완료)
- ✅ **feature/linkerd 브랜치** Infrastructure as Code manifest 활용
- ✅ **외부 접근 가능한 관찰가능성 플랫폼** 완성

**성공 기준**
- 🚀 **30분 내 전체 스택 동작**: Prometheus + Grafana + Bookinfo + 트래픽 생성
- 🌐 **외부 접근 완료**: http://Master EC2 Public_IP:30080 (Bookinfo), http://Master EC2 Public_IP:30300 (Grafana)
- 📊 **실시간 모니터링**: 메트릭 수집 및 대시보드 정상 동작
...
### **핵심 설계 결정**
1. **Git 기반 배포**: feature/linkerd 브랜치의 manifest 파일 활용
2. **마스터노드 집중**: Self-managed 환경 최적화 패턴 적용
3. **HostPort 전략**: NodePort 제약을 근본적으로 우회
4. **자동화 스크립트**: 수동 개입 최소화
